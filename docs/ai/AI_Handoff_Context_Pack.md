...
- **RAG layer:** Amazon Bedrock Knowledge Bases
...
Use **Amazon Bedrock Knowledge Bases** to simplify managed RAG instead of building a custom ingestion + embedding + vector indexing + retrieval orchestration pipeline.

### Use S3 Vectors, not OpenSearch Serverless
The user decided to optimize for time and cost. Therefore:
- prefer **S3 Vectors** as the vector store for Bedrock Knowledge Bases
- do **not** choose OpenSearch Serverless for MVP unless there is a compelling implementation reason
