...
## Current Status
- Current phase: **Phase 4 - Polish and Submission Prep**
- Current focus: **Phase 3 deployment is now complete with a live Lambda/API Gateway backend, DynamoDB persistence, OpenSearch connectivity, and local Streamlit validation against the cloud API; remaining work shifts to final README polish, screenshots, demo prep, and submission packaging**
- Last updated: **2026-04-18 (Asia/Ho_Chi_Minh)**
- Overall state: **Phase 3 has been executed end-to-end: Terraform applied successfully, the backend is live at API Gateway URL `https://sldzem1pol.execute-api.us-east-1.amazonaws.com/`, AOSS policy access was updated to trust the Lambda role, live health/knowledge/order flows were verified, DynamoDB persistence was confirmed, and deployment evidence was captured under `evidence/phase3/`.**
...
- [x] Level 300 minimum viable features completed
- [x] Level 200 minimum viable deployment completed
...
#### 1. IaC
- [x] Create IaC skeleton
- [x] Define Lambda resources
- [x] Define API Gateway resources
- [x] Define DynamoDB resources
- [x] Define S3 resources

#### 2. Deployment
- [x] Deploy backend to AWS
- [x] Verify live API endpoint
- [x] Connect frontend to cloud backend

#### 3. CI/CD
- [x] Add GitHub Actions workflow
- [x] Add lint/test step
- [x] Add package/deploy step if feasible
...
- `requirements.txt` now includes `mangum` for the Lambda deployment path
- `app/backend/handler.py` now exposes the real Lambda entrypoint via `handler = Mangum(app)`
- `scripts/package_lambda.py` now builds `artifacts/backend-lambda.zip` as a portable deployment artifact
- Terraform files added under `infra/terraform/` for provider config, variables, locals, IAM, DynamoDB, S3, Lambda, API Gateway, outputs, and example tfvars
- `infra/terraform/terraform.tfvars.example` now locks the Phase 3 defaults to `python3.12`, `512 MB`, `30s`, and `14` day log retention
- `infra/terraform/.terraform.lock.hcl` was generated by `terraform init`
- GitHub Actions workflows added: `.github/workflows/ci.yml` and `.github/workflows/cd-support.yml`
- Deployment documentation added: `docs/architecture/phase3-deployment-notes.md`
- `README.md` now documents deployment scope, target vs fallback architecture, Terraform input to Lambda env mapping, deployment defaults, and verification steps
- `python scripts/package_lambda.py` completed successfully and created `artifacts/backend-lambda.zip`
- `terraform -chdir=infra/terraform init -backend=false` and `terraform -chdir=infra/terraform validate` completed successfully
- `terraform -chdir=infra/terraform apply -auto-approve` succeeded after importing the existing DynamoDB table `agentic-commerce-conversation`
- Live backend outputs were captured in `evidence/phase3/terraform-output.json`
- Live API Gateway URL is `https://sldzem1pol.execute-api.us-east-1.amazonaws.com/`
- AOSS data access policy `easy-ecommercesupportsearch` was updated to include Lambda role `arn:aws:iam::029930584678:role/agentic-commerce-backend-dev-role`
- Health endpoint verification evidence saved to `evidence/phase3/health.json`
- Knowledge success evidence saved to `evidence/phase3/knowledge-success.json`
- Knowledge fallback evidence saved to `evidence/phase3/knowledge-fallback.json`
- Order workflow success evidence saved to `evidence/phase3/order-success.json`
- DynamoDB persistence evidence saved to `evidence/phase3/dynamodb-query.json`
- CloudWatch execution evidence saved to `evidence/phase3/cloudwatch-snippet.txt`
- Local Streamlit frontend was restarted against the cloud backend and response evidence was captured in `evidence/phase3/frontend-cloud.html`
- Phase 3 deployment uncovered and fixed three real issues: oversized Windows-built Lambda artifact, reserved `AWS_REGION` Lambda env injection, and API Gateway stage throttling defaulting to zero
- Lambda packaging now includes runtime-only dependencies plus `data/mock/orders.json` so the order workflow works in the deployed environment

---

## Next 3 Tasks
1. Finalize README and submission documentation using the captured Phase 3 deployment evidence
2. Add final architecture screenshots and package the strongest evidence for demo/interview discussion
3. Record the demo and prepare the final submission bundle
